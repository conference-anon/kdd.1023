...
class PointCNN:
    def __init__(self, points, features, is_training, setting, point_dims=3, reshape_features=True):
        ...
        if features is None:
            ...
            if reshape_features:
                features = tf.reshape(features, (N, -1, setting.data_dim - point_dims), name='features_reshape')
            else:
                assert len(features.shape) == 3
                with tf.control_dependencies([
                    tf.assert_equal(tf.shape(features)[:2],
                                    tf.shape(points)[:2])
                ]):
        ...
        for layer_idx, layer_param in enumerate(xconv_params):
            ...
            if P == -1 or (layer_idx > 0 and P == xconv_params[layer_idx - 1]['P']):
                ...
                if setting.sampling == 'fps':
                    ...
                    raise ValueError('Unknown sampling method! %s' % setting.sampling)
